...
for i in range(len(main_info.index[:])):
...
del main_info['index']
while True:
    ord = input('please type your order!')
    try:
        if ord == 'exit':
            break
        elif ord == 'append':
            ...
            main_info = main_info.append(newrow, ignore_index=True)
            newrow['Amount'] = do.DIV(newrow, div, initial_info)
            newrow['Unit'] = newrow['Units']
            del newrow['Units']
            saved = saved.append(pd.DataFrame(newrow).explode('Unit'), ignore_index=True)
            main_info = main_info.sort_values(by='Time').reset_index()
            del main_info['index']
            saved = saved.sort_values(by='Time').reset_index()
            del saved['index']
            m1 = main_info.sort_values(by='id').reset_index()
            del m1['index']
            s1 = saved.sort_values(by='id').reset_index()
            del s1['index']
